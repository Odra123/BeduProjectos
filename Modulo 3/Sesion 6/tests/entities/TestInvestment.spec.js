const { getFinalAmountAfterTaxes } = require("../../entities/Investment");
...
describe('Investment', () => {
    describe('Add new investment', () => {
        ...
        it('Get final amount after taxes', () => {
            ...
            const expectedId = '123e4567-e89b-12d3-a456-426655440000';
            const repository = new InvestmentRepositoryStub(expectedId);
            const expectedFinalAmount = startingAmount * (1 + (((interest / 100) / 360) * durationDays));
           // const agregarInvestmen = Investment.addInvestment(name, description, interest, startingAmount, startDate, durationDays, repository);
           const finalamountexpected = expectedFinalAmount- (((expectedFinalAmount-0.01)*0.0192)+0);
            const investment = Investment.getFinalAmountAfterTaxes(interest, startingAmount, durationDays);
            //const investment = Investment.getFinalAmountAfterTaxes();

            expect(investment).toEqual(finalamountexpected);
        })
